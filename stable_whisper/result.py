...
class WhisperResult:
    ...
    def regroup(
        self, regroup_algo: str = None, verbose: bool = False, only_show: bool = False
    ):
        """
        ...
        if isinstance(regroup_algo, str):
            ...
            for method in regroup_algo.split("_"):
                method, args = (
                    method.split("=", maxsplit=1) if "=" in method else (method, "")
                )
                if method not in methods:
                    raise NotImplementedError(
                        f"{method} is not one of the available methods: {tuple(methods.keys())}"
                    )
